...
public:
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        ...
        int alen = get_length(headA), blen = get_length(headB);
        // A and B are aligned on their tail so moving along the longest until it contains the length of
        // the shortest elements and they will be aligned
        if (alen > blen) headA = node_at(headA, alen-blen);
        else if (alen < blen) headB = node_at(headB, blen-alen);
